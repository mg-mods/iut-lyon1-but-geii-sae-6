  ...
  while (true)
  {
    ...
    if (InputUser.length() >= passwordLength)
    {
      ...
      if (xSemaphoreTake(xSerialMutex, portMAX_DELAY) == pdTRUE)
      {
        ...
        {
          // Préfixe exact à vérifier
          String FormatStatePass = "str/" + IDhub + "/" + IDdigi + "/StatePass/";
          Serial.println("FormatStatePass:" + FormatStatePass);
          String StatPass = "";
          if (resp.startsWith(FormatStatePass))
          {
            StatPass = resp.substring(FormatStatePass.length());
            StatPass.trim(); // enlever espaces ou retour chariot
          }
          Serial.println("resp:" + resp);
          Serial.println("statepass:" + StatPass);
    ...
    if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial())
    {
      ...
      if (xSemaphoreTake(xSerialMutex, portMAX_DELAY) == pdTRUE)
      {
        ...
        else
        {
          ...
          if (resp.substring(FormatStateRFID.length()) == "true")
          {
            ev = EVENT_GOOD;
